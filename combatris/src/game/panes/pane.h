...
class TextPane : public Pane {
 public:
  ...
  TextPane(SDL_Renderer* renderer, int x, int y, const std::string& text, const std::shared_ptr<Assets>& assets)
      : Pane(renderer, x, y, assets), caption_texture_(utility::Texture(renderer_, assets_->GetFont(Bold25), text, Color::White)) {}
  ...
  void SetCenteredText(const std::string& text, Color color = Color::SteelGray, const Font& font = Bold45) {
    lines_.resize(1);
    auto& line = lines_[0];
    line = utility::Texture(renderer_, assets_->GetFont(font), text, color);
  ...
  void SetCenteredText(const std::string& text1, Color color1, const std::string& text2, Color color2) {
    lines_.resize(2);
    auto& line1 = lines_[0];
    line1 = utility::Texture(renderer_, assets_->GetFont(Bold25), text1, color1);
    line1.SetX(line1.center_x(kBoxWidth));
    line1.SetY(caption_texture_.height() + 15);

    auto& line2 = lines_[1];
    line2 = utility::Texture(renderer_, assets_->GetFont(Bold25), text2, color2);
 ...
 protected:
  std::vector<utility::Texture> lines_;
  utility::Texture caption_texture_;
